...
Вот суммаризация этогов видео [Обновление1СЧерезGIT.summary.md](Обновление1СЧерезGIT.summary.md)
Ну и транскрибация, можно скормить ЛЛМ и проанализировать в интересущем разрезе [Обновление1СЧерезGIT.speaker_transcriptions.csv](Обновление1СЧерезGIT.speaker_transcriptions.csv)

Кратко метод заключается в том, что работа происходит с выгрузкой конфигурации 1С в файлы, а эта выгрузка индексируется в GIT. Создается две ветки, вендора - `type` и измененной конфигурации `dev`. Названия условные, можно как угодно. Первый коммит в `type` иходная версия поставщика, второй коммит в `type` обновленная версия постащика. От первого коммита создать ветку `dev`, в нее выгрузить доработанную версию. Получаем структуру, которая аналогична той что в конфигураторе. Не помню как точно, вроде "Старая конфигурация поставщика, Новая конфигурация поставщика, Основная конфигурация".
Потом выполняется команда `git mergr type`, и все что может объединиться объединяется. Для остального надо разрешить конфликты в ручную или скриптами или ИИ. Про ИИ возможно, 99% что да, но сам, увы не попробовал. ИИ мне фиксил то, что я вручную накосячил, думаю он и с мерджем справился бы за несколько итераций.
...
- `+` GIT репозитарий в состоянии MERGE может существовать бесконечно долго. Если закрыть конфигуратор или окно объединения все результаты будут потеряны. В конфигураторе, конечно, есть функция сохранять настройки объединения в файл, но вобще это геморой;
...
- `+` Объединение измененных ролей. В конфигураторе вобще нельзя сравнить роли. В git в xml все видно. Вобще, типовые роли менять не надо! За это в ад, водить мусоровоз до конца времен. Но бывает.
...
Кто-то при обновлении наделал в модулях моржей, все модули в тегах MRG. После чего накатил еще пару релизов частично, без поддержки. Т.е было даже не понятно, какая на самом делел там версия.
Моржей я вычистил и определил базовый релиз за пару дней. ИИ это сделал бы за пару часов наверное, но тогда я не умел.
Перечень изменений и их цель нигде не были зафиксированы, поэтому почти все узнавал от пользователей, когда ломал. Документировал, писал тесты, рефекторил. Плакал о загубленной карме.
В плюсе было только то, что использовался минимум функционала, фактически только CRM, выписка документов и некоторые отчеты. Это позволило скипнуть полное обновление через 16 релизов, и обойтись тремя. А подсказал мне это ИИ. При починке одного из обработчиков обновлений, стало понятно, что обработчики верси 3.0.12 допускают обновление с 1.6.25. До нее было два релиза, и третий на 3.0.12. Т.к. функционала использовался минимум, я попросил проанализироват, есть ли удаляемые реквизиты в используемых объектах, ИИ таких не нашел. И я рискнул обновиться сразу через 12 релизов, и получилось. Если бы не получилось, попросил бы исследовать подробнее, через какие можно. Это везение, что разработчики УНФ так сделали.
...
Это можно делать итеративно: `git merge` —> посмотреть что не так —> `git merge --abort` —> поправить/отрефакторить —> снова `git merge`. Так по кругу, до достижения удовлетворительного результата. Понятное дело, что для ИИ поправить/отрефакторить основное занятие.

## Сборка ветки поставщика `type`

Я в этом проекте не пользовался скриптами автоматической сборки, т.к обошелся малой кровью, и вместо 16 релизов было только 3. Загрузил руками. Но вобще, есть решения:
...
- [docs/КачайСтройВеткуПоставщика.os](docs/КачайСтройВеткуПоставщика.os) - Вот это свое. Ищет на сайте 1С следующий релиз, скачивает, устанавливает, загружает в GIT, коммитит. Через пару часов имеем последовательную ветку релизов. Писано еще руками, уже год как не использовал, может тоже не работает.
...
И попросил написать скрипт, который автоматически разрешает конфликты, правила для которых понятны:
...
Тесты предельно примитивные, без фремворков. Каждый тест это процедура, которая что-то проверяет. Если проверка не проходит, процедура вызывает исключение. Если проходит - ничего не делает. Процедура самодостаточна, можно использовать без инфраструктуры, просто вызвать.
...
Большинство тестов написано ИИ. Списки тестов поддерживает ИИ самостоятельно, просто фунция возвращающая список имен тестов, обновляется при добавлении каждого нового теста. Тесты писал по ходу столкновения с проблемами. Иногда openspec добавляет тесты, но обычно это фуфло, сношу их.
...
Для задач по доработке в основно использовал Gemini и Sonnet. Разницы в качестве не заметил. Для скриптов и аналитики кода довольно много использвал grok 4.1, он был очень дешевый, считай бесплатно работал, качество было ок. Grock 4.2 уже сильно дороже, не имеет смысла. Зато появился composer2, скрипты и анализ кода делает хорошо, да 1С делает нормально, часто добивал им задачи, когда в лимиты CLAUDE упирался.

MCP Нет. Ставил, но не привык. Долгая индексация и баги большой оверхэд. Контроль состояния докера и MCP сервисов перед каждой сессией. Дополнительная документация. Массовые изменения конфигурации вселяли сомнения по корректности использования ранее индексированных данных. Времени разбираться не было, надо было пилить обновление, а модели вполне норм работали без MCP и я забил. Соннет не ошибается в синтаксисе 1С. Gemini почти не ошибается. Composer2 тоже ок работает на задачах типа восстановления снесенного обновлением функционала.
...
Прижился [context7](https://context7.com). Настроил и работает. Модели юзают когда задачи не 1С, скрипты и инструменты пишут. Пару раз и для 1С что-то смотрели, ну знаю уж помогло им это или нет.

Для новых фич использовал `openspec`
